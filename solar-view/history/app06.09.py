...
ALL_PLANTS = [
    ...
    "RES_ENERGY_PVPP", "Luxus_Energy_PVPP", "IF - Saftica CEF 1 MW", "Trecon","Foton Plus Urzica", "CEF Dalga", "Albesti", "Skipass", "ML MV Green Energy", "CEF Ruseni", "RAAL_PB_7.371MWp_6.02MW", "CEF Miercurea Ciuc", "Catalina", "PROINTELIGENTA CRISTESTI 13", "DAMIANA Cristesti 12", "GDI Harvesting CEF Rogojeni", "CEF. Unirea", "CEF Silistea"
...
PLANTS_METHOD = {
    ...
    "CEF. Unirea":                 "smartlogger",
    "CEF Silistea":                "smartlogger",
}
        ...
        if not st.session_state["curtail_authenticated"]:
            ...
            def plant_to_listener(plant: str) -> str:
                ...
                if m == "united":
                    return "united"
                if m == "trecon":
                    return {"Trecon": "trecon",
                            "Foton Plus Urzica": "urzica",
                            "IF - Saftica CEF 1 MW": "gcit"}.get(plant, "trecon")
                return "fusion"

            def send_curtail_command(action: str, plants: list = None, pct: float = 0.0):
                """
                Trimite comanda curtail/restore in Supabase — UN RAND PER LISTENER.
                Toate randurile se insereaza in acelasi click => pornire simultana.
                ...
                try:
                    ...
                    # Grupeaza plantele pe listener; kw calculat PER GRUP
                    groups = {}
                    skipped = []
                    for plant in target_plants:
                        ...
                        if kw_val is None:
                            ...
                            continue
                        lst = plant_to_listener(plant)
                        g = groups.setdefault(lst, {"plants": [], "kw": 0.0})
                        g["plants"].append(plant)
                        if PLANTS_METHOD.get(plant) not in ("trecon", "shared") and g["kw"] == 0.0 and kw_val > 0:
                            g["kw"] = kw_val

                    if not groups:
                        return False, f"⚠️ No valid plants — all have kw_max undefined."

                    # Idempotency per (action, listener) — double-click protection
                    now_utc = datetime.now(ZoneInfo("UTC"))
                    recent = supabase.table("curtail_commands") \
                        .select("id,created_at,listener") \
                        .eq("action", action) \
                        .in_("status", ["pending", "running"]) \
                        .order("created_at", desc=True) \
                        .limit(20) \
                    ...
                    for r in (recent.data or []):
                        try:
                            age_s = (now_utc - datetime.fromisoformat(
                                r["created_at"].replace("Z", "+00:00"))).total_seconds()
                            if age_s < 60 and r.get("listener") in groups:
                                blocked.add(r.get("listener"))
                        except Exception:
                            pass

                    sent = []
                    for lst, g in groups.items():
                        if lst in blocked:
                            skipped.append(f"{lst} ({action} in progress <60s)")
                            continue
                        payload = {
                            "action":      action,
                            "kw":          g["kw"],
                        ...
                        if result.data:
                            sent.append(f"{lst}:`{str(result.data[0]['id'])[:8]}`")

                    if not sent:
                        return False, f"⚠️ Nicio comanda trimisa. Skipped: {', '.join(skipped)}"
                    msg = f"Sent {len(sent)} command(s): " + ", ".join(sent)
                    if skipped:
