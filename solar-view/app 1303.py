...
def main():
    ...
    with tab2:
        ...
        # ---- Schedule helpers ----
        def get_schedules(status_filter=None):
            try:
                sb2 = create_client(SUPABASE_URL, SUPABASE_KEY)
                q = sb2.table('curtail_schedule').select('*').order('start_at', desc=False)
                if status_filter:
        ...
        def create_schedule(plants, start_at, end_at):
            try:
                sb2 = create_client(SUPABASE_URL, SUPABASE_KEY)
                payload = {
                    "plants": plants,
                ...
                sb2.table('curtail_schedule').insert(payload).execute()
                return True, "Programare salvată!"
            except Exception as e:
        ...
        def cancel_schedule(schedule_id):
            try:
                sb2 = create_client(SUPABASE_URL, SUPABASE_KEY)
                sb2.table('curtail_schedule').update({"status": "cancelled"}).eq("id", schedule_id).execute()
                return True
            except Exception:
                return False

        # ---- Schedule UI ----
        with st.expander("📅 Programare curtailment", expanded=False):
            st.markdown("**Programează curtailment automat pe interval orar**")

            BUC = ZoneInfo("Europe/Bucharest")
            now_buc = datetime.now(BUC)

            col_p, col_d = st.columns([2, 1])
            with col_p:
                sel_all_sched = st.checkbox("Toate centralele", value=True, key="sched_all")
                if sel_all_sched:
                    sched_plants = ALL_PLANTS
                else:
                    sched_plants = st.multiselect("Centrale:", ALL_PLANTS, default=[], key="sched_plants")
            with col_d:
                sched_date = st.date_input("Data:", value=now_buc.date(), min_value=now_buc.date(), key="sched_date")

            col_t1, col_t2 = st.columns(2)
            with col_t1:
                sched_start = st.time_input("Ora start (curtail):", value=now_buc.replace(minute=0, second=0, microsecond=0).time(), key="sched_start", step=300)
            with col_t2:
                sched_end = st.time_input("Ora stop (restore):", value=now_buc.replace(hour=min(now_buc.hour+3, 23), minute=0, second=0, microsecond=0).time(), key="sched_end", step=300)

            start_dt = datetime.combine(sched_date, sched_start, tzinfo=BUC)
            end_dt   = datetime.combine(sched_date, sched_end,   tzinfo=BUC)

            valid = True
            if end_dt <= start_dt:
                st.error("⚠️ Ora stop trebuie să fie după ora start.")
                valid = False
            if start_dt < now_buc:
                st.warning("⚠️ Ora start e în trecut.")

            if valid and sched_plants:
                st.info(f"📋 **{len(sched_plants)} centrale** | Curtail la **{sched_start.strftime('%H:%M')}** → Restore la **{sched_end.strftime('%H:%M')}** | {sched_date.strftime('%d.%m.%Y')}")
                if st.button("💾 Salvează programare", key="sched_save", use_container_width=True):
                    ok, msg = create_schedule(sched_plants, start_dt, end_dt)
                    if ok:
                        st.success(msg)
                        st.cache_data.clear()
                        st.rerun()
            ...
            # Lista schedule-uri active/viitoare
            st.markdown("---")
            st.markdown("**Programări active / viitoare:**")
            schedules = get_schedules(status_filter=["pending", "active"])
            if not schedules:
                st.caption("Nicio programare activă.")
            else:
                for s in schedules:
                    sid    = s.get("id")
                    status = s.get("status", "?")
                    s_start = s.get("start_at", "")[:16].replace("T", " ")
                    s_end   = s.get("end_at",   "")[:16].replace("T", " ")
                    s_plants = s.get("plants", [])
                    n = len(s_plants) if isinstance(s_plants, list) else "?"
                    icon = "⏳" if status == "pending" else "🔴"
                    col_si, col_sb = st.columns([4, 1])
                    with col_si:
                        st.markdown(f"{icon} **{status.upper()}** | {s_start} → {s_end} | {n} centrale")
                        st.caption(", ".join(s_plants) if isinstance(s_plants, list) else "")
                    with col_sb:
                        if st.button("❌ Anulează", key=f"cancel_{sid}"):
                            if cancel_schedule(sid):
                                st.success("Anulat")
                                st.rerun()

            # Overwrite instant
            st.markdown("---")
            st.markdown("**⚡ Overwrite instant** — anulează schedule activ și trimite comandă acum")
            col_ow1, col_ow2 = st.columns(2)
            with col_ow1:
                if st.button("🔴 CURTAIL ACUM (override)", key="ow_curtail", use_container_width=True):
                    for s in get_schedules(status_filter=["pending", "active"]):
                        cancel_schedule(s["id"])
                    ok, msg = send_curtail_command("curtail", ALL_PLANTS)
                    if ok:
                        st.success(f"✅ Schedule anulat + {msg}")
                        st.cache_data.clear()
                        st.rerun()
                    else:
                        st.error(msg)
            with col_ow2:
                if st.button("🟢 RESTORE ACUM (override)", key="ow_restore", use_container_width=True):
                    for s in get_schedules(status_filter=["pending", "active"]):
                        cancel_schedule(s["id"])
                    ok, msg = send_curtail_command("restore", ALL_PLANTS)
                    if ok:
